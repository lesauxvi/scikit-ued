...
from ... import Crystal, repeated_array, minimum_image_distance
    ...
    def test_sim_mesh_range(self):
        ...
        for crystal in self.crystals:
            with self.subTest('Crystal = {}'.format(crystal.source)):
                X, Y = sim_mesh(crystal, resolution = (256, 256))
                per_x, per_y, _ = crystal.periodicity

                dx, dy = X[1,1] - X[0,0], Y[1,1] - Y[0,0]

                # Rounding down to 5 digits because of floating errors
                ncellx = round((X.max() - X.min() + dx) / per_x, ndigits = 8)
                ncelly = round((Y.max() - Y.min() + dy) / per_y, ndigits = 8)

                self.assertAlmostEqual(ncellx, round(ncellx))
                self.assertAlmostEqual(ncelly, round(ncelly))        
    ...
    def test_sim_mesh_spacing(self):
        ...
        for crystal in self.crystals:
            with self.subTest('Crystal = {}'.format(crystal.source)):
                ...
                self.assertAlmostEqual(ndx, int(round(ndx)))
                self.assertAlmostEqual(ndy, int(round(ndy)))
...
class TestWeakPhase(unittest.TestCase):
    ...
    def test_intensity_conservation(self):
        ...
        for crystal in self.crystals:
            with self.subTest('Crystal = {}'.format(crystal.source)):
                X, Y = sim_mesh(crystal, resolution = (128, 128))
